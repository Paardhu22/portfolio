...
import { dragBoost, dragSpan, dragThrottle, scrollsPage, travelAction, type TravelAction } from '@/components/input';
...
type GridScanProps = {
  ...
  shakeAmount?: number;
  /** Overall strength of the idle hover: drift, bank and pitch. 0 disables it. */
  hoverAmount?: number;
  /** Rate of the hover drift. Below ~0.6 it reads as becalmed, above ~2 as nervous. */
  hoverSpeed?: number;
  doubleTapMs?: number;
  /** Depth at which the corridor ends. Left undefined, it runs on forever. */
  travelEnd?: number;
  /** Units over which the throttle is eased away on approach to travelEnd. */
  arrivalRamp?: number;
  /** Fires when you settle at the end, and again when you pull away from it. */
  onArrive?: (arrived: boolean) => void;
  /** Ascending tunnel depths at which checkpoints activate. */
  checkpointDepths?: number[];
  /** Fires only when the active checkpoint changes; -1 means none. */
...
uniform vec2 uShake;
uniform float uScanStatic;
uniform float uScanAnchor;
varying vec2 vUv;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  float hitIsY = 1.0;
    for (int i = 0; i < 4; i++)
    {
        ...
        float depthBoost = smoothstep(0.0, 3.0, h.z - uTravel);
  ...
  float lineMask = max(primaryMask, altMask);
    ...
    float scanZ = mix(phase * scanZMax, uScanAnchor, uScanStatic);
    float dz = abs(hitZ - scanZ);
    ...
    float phaseWindow = mix(headFade * tailFade, 1.0, uScanStatic);
...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  lineThickness = 1,
  linesColor = '#2F293A',
  scanColor = '#D9D6DE', // keep in sync with --color-accent in app/globals.css
  scanOpacity = 0.4,
  gridScale = 0.1,
  lineStyle = 'solid',
  lineJitter = 0.1,
  ...
  const travelDirRef = useRef(0);
  const travelVelRef = useRef(0);
  const travelAccelRef = useRef(0);
  const travelSpeedRef = useRef(travelSpeed);
  travelSpeedRef.current = travelSpeed;
  const boostRef = useRef(0);
  const shakeRef = useRef(0);
  const shakeVelRef = useRef(0);
  ...
  const travelEndRef = useRef(travelEnd);
  travelEndRef.current = travelEnd;
  const arrivalRampRef = useRef(arrivalRamp);
  arrivalRampRef.current = arrivalRamp;
  const onArriveRef = useRef(onArrive);
  onArriveRef.current = onArrive;
  const arrivedRef = useRef(false);
  const reduceMotionRef = useRef(false);
  const checkpointDepthsRef = useRef(checkpointDepths);
  checkpointDepthsRef.current = checkpointDepths;
  const onCheckpointChangeRef = useRef(onCheckpointChange);
  onCheckpointChangeRef.current = onCheckpointChange;
  const activeCheckpointRef = useRef(-1);
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      uTravel: { value: travelRef.current },
      uShake: { value: new THREE.Vector2(0, 0) },
      uScanStatic: { value: scanStatic ? 1 : 0 },
      uScanAnchor: { value: scanAnchor }
    };
    ...
    const tick = () => {
      ...
      // Hidden behind another world: hold position and stop spending the GPU.
      ...
      const boostScale = 1 + boostRef.current * (boostMultiplierRef.current - 1);

      // Approach: ease the throttle away over the last stretch rather than
      // clipping the position, so you decelerate into the end instead of
      // hitting it. The pitch spring reads that deceleration and dips the nose
      // on arrival at no extra cost.
      const end = travelEndRef.current;
      let dir = travelDirRef.current;
      if (end !== undefined && dir > 0) {
        const r = THREE.MathUtils.clamp(
          (end - travelRef.current) / Math.max(1e-3, arrivalRampRef.current),
          0,
          1
        );
        dir *= r * r * (3 - 2 * r);
      ...
      const travelSm = smoothDampFloat(
        travelVelRef.current,
        dir * travelSpeedRef.current * boostScale,
        { v: travelAccelRef.current },
        THREE.MathUtils.lerp(0.35, 0.14, Math.min(1, boostRef.current)),
        maxSpeed,
        dt
      );
      travelVelRef.current = travelSm.value;
      travelAccelRef.current = travelSm.v;
      travelRef.current += travelVelRef.current * dt;

      // Bounded at both ends: the corridor starts where it starts, and whatever
      // the easing leaves over can't carry you past the end.
      if (travelRef.current <= 0) {
        travelRef.current = 0;
        if (travelVelRef.current < 0) travelVelRef.current = 0;
      }
      if (end !== undefined && travelRef.current >= end) {
        travelRef.current = end;
        if (travelVelRef.current > 0) travelVelRef.current = 0;
      }
      ...
      if (end !== undefined) {
        ...
        const atEnd = end - travelRef.current < 0.02 && Math.abs(travelVelRef.current) < 0.02;
        if (atEnd !== arrivedRef.current) {
          arrivedRef.current = atEnd;
          onArriveRef.current?.(atEnd);
        }
      }

      const depths = checkpointDepthsRef.current;
      if (depths) {
        let idx = -1;
        for (let i = 0; i < depths.length; i++) {
          if (travelRef.current >= depths[i]) idx = i;
        }
        if (idx !== activeCheckpointRef.current) {
          activeCheckpointRef.current = idx;
          onCheckpointChangeRef.current?.(idx);
        }
      }

      const shakeSm = smoothDampFloat(
        shakeRef.current,
        boostRef.current,
        { v: shakeVelRef.current },
        0.18,
        maxSpeed,
        dt
      );
      shakeRef.current = shakeSm.value;
      shakeVelRef.current = shakeSm.v;
      // A drifting, banking first-person corridor is exactly the content that
      // provokes motion sickness, so the ambient layers drop out entirely when
      // the visitor has asked for less. Travel stays - it's the navigation.
      const rm = reduceMotionRef.current ? 0 : 1;

      const amp = rm * shakeRef.current * shakeAmountRef.current;
      const ts = now / 1000;

      // Hover: three reads of "this thing has mass and is holding station".
      //   drift - a slow wander, so it never sits perfectly still
      //   bank  - roll into that wander, from its analytic derivative
      //   pitch - a spring on longitudinal accel: nose up under thrust, nose
      //           down on the brakes, one overshoot past level on release
      const hs = hoverSpeedRef.current;
      const th = ts * hs;
      // Incommensurate frequencies, so the pattern never visibly repeats.
      const driftX = Math.sin(th * 0.37) * 0.6 + Math.sin(th * 0.83) * 0.4;
      const driftY = Math.cos(th * 0.29) * 0.6 + Math.cos(th * 0.61) * 0.4;
      // d(driftX)/dt - the bank leads the drift, the way it does in a real turn.
      const driftXVel = hs * (0.37 * Math.cos(th * 0.37) * 0.6 + 0.83 * Math.cos(th * 0.83) * 0.4);

      // Steadier at speed, and pulled back under boost so it can't muddy the shake.
      const speedFrac = Math.min(1, Math.abs(travelVelRef.current) / Math.max(1e-3, travelSpeedRef.current));
      const hover = rm * hoverAmountRef.current * (1 - 0.45 * speedFrac) * (1 - 0.5 * shakeRef.current);

      const accelSignal = Math.tanh(travelAccelRef.current / Math.max(1e-3, travelSpeedRef.current * 4));
      // Damping ratio ~0.58: underdamped enough for one visible overshoot,
      // damped enough that it settles instead of wobbling.
      pitchVelRef.current += (accelSignal - pitchRef.current) * 42 * dt - pitchVelRef.current * 7.5 * dt;
      pitchRef.current += pitchVelRef.current * dt;
      const pitch = rm * pitchRef.current * hoverAmountRef.current;

      material.uniforms.uShake.value.set(
        amp * (Math.sin(ts * 16.5) * 0.65 + Math.sin(ts * 27.3) * 0.35) + hover * 0.008 * driftX,
        amp * (Math.cos(ts * 14.1) * 0.65 + Math.cos(ts * 23.7) * 0.35) + hover * 0.006 * driftY
      );
      material.uniforms.uSkew.value.x += hover * 0.012 * driftX;
      material.uniforms.uSkew.value.y += pitch * 0.05;
  ...
  useEffect(() => {
    ...
    if (m) {
      ...
      u.uScanStatic.value = scanStatic ? 1 : 0;
      u.uScanAnchor.value = scanAnchor;
    }
    if (bloomRef.current) {
  ...
  useEffect(() => {
    // Physical key (e.code) -> action, so holding W and tapping ArrowUp doesn't
    // stop you dead when you let the arrow go: forward stays held while any
    // forward key is still down.
    const held = new Map<string, TravelAction>();
    const lastTap: Record<TravelAction, number> = { forward: 0, back: 0 };
    const boosted: Record<TravelAction, boolean> = { forward: false, back: false };
    const apply = () => {
      const actions = new Set(held.values());
      const fwd = actions.has('forward');
      const back = actions.has('back');
      if (!fwd) boosted.forward = false;
      if (!back) boosted.back = false;
      const dir = fwd === back ? 0 : fwd ? 1 : -1;
      travelDirRef.current = dir;
      const action: TravelAction | null = dir === 1 ? 'forward' : dir === -1 ? 'back' : null;
      boostRef.current = action && boosted[action] ? 1 : 0;
    };
    const onKeyDown = (e: KeyboardEvent) => {
      const action = travelAction(e);
      if (!action) return;
      if (scrollsPage(e)) e.preventDefault();
      if (e.repeat) return;
      const now = performance.now();
      // Timed per action, not per key, so ArrowUp then W still reads as a double tap.
      if (now - lastTap[action] < doubleTapMsRef.current) boosted[action] = true;
      lastTap[action] = now;
  ...
  useEffect(() => {
    ...
    // One finger owns the throttle; later touches are ignored rather than
    // fighting it. Mouse is excluded on purpose - the desktop already has a
    // better control, and hijacking drag would break the look-parallax.
    let activeId: number | null = null;
    let originY = 0;
    let span = dragSpan();

    const setThrottle = (t: number) => {
      travelDirRef.current = t;
      boostRef.current = dragBoost(t);
    };
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      setThrottle(dragThrottle(originY - e.clientY, span));
    };

    const onPointerEnd = (e: PointerEvent) => {
      if (e.pointerId !== activeId) return;
      activeId = null;
      // Spring-loaded: releasing returns the lever to idle.
      setThrottle(0);
    };
  ...
  return (
    <div ref={containerRef} className={`relative w-full h-full touch-none select-none overflow-hidden ${className ?? ''}`} style={style}>
      {showPreview && (
